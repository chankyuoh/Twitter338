...
class MyStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, tweet):
        if tweet_contains_video(tweet):
            ...
            if executeQuery(checkexist) == 0 and (not tweet.retweeted) and ('RT @' not in tweet.text):
...
def get_users_tweets_and_links(tweet):
    ...
    for user_tweet in user_tweets:
        if tweet_contains_video(user_tweet) and (not tweet.retweeted) and ('RT @' not in tweet.text):
